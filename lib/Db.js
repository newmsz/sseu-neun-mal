...
function loadToWhitelist(name) {
    ...
    var list = fs.readFileSync(__dirname + '/../db/white/' + name).toString().split(',');
...
function loadToBlacklist(name) {
    if(!name.match(/\.db$/)) return;
    var list = fs.readFileSync(__dirname + '/../db/black/' + name).toString().split(',');
...
function loadJosa() {
    var list = fs.readFileSync(__dirname + '/../db/josa.db').toString().split(',');
    for(var i=0; i<list.length; i++) {
...
fs.readdirSync(__dirname + '/../db/white').forEach(loadToWhitelist);
fs.readdirSync(__dirname + '/../db/black').forEach(loadToBlacklist);
loadJosa();
